          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              <div style={{
                ...
                <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: 1, background: "var(--border-color)" }} />
                <span style={{ fontFamily: "var(--font-noto-sans-jp)", fontWeight: 100, fontSize: 60, color: "var(--text-primary)", lineHeight: 1, position: "relative" }}>
                  {item.kanji}
                </span>
              </div>
              <div>
                <h2 style={{ fontFamily: "var(--font-cormorant)", fontSize: 28, fontWeight: 600, color: "var(--text-primary)", marginBottom: 4 }}>
                  {item.meaning}
                </h2>
                <p style={{ fontFamily: "var(--font-inter)", fontSize: 11, color: "var(--text-secondary)", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 10 }}>
            ...
            {activeTab === 'info' && (
              <>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  {[
                    { label: "On'yomi", data: item.onyomi, bg: "rgba(165, 180, 252, 0.1)", pillBg: "#e0e7ff", pillText: "#3730a3", labelColor: "#818cf8" },
                    { label: "Kun'yomi", data: item.kunyomi, bg: "rgba(74, 222, 128, 0.1)", pillBg: "#dcfce7", pillText: "#166534", labelColor: "#4ade80" },
                  ].map(({ label, data, bg, pillBg, pillText, labelColor }) => (
                    <div key={label} style={{ background: bg, borderRadius: 16, padding: "16px 18px" }}>
                      <p style={{ fontFamily: "var(--font-inter)", fontSize: 10, fontWeight: 600, color: labelColor, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 10 }}>
                        {label}
                      </p>
                      {data.length > 0 ? (
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {data.map((r: string, i: number) => (
                            <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 4, background: pillBg, color: pillText, borderRadius: 99, padding: "3px 10px", fontSize: 13, fontFamily: "var(--font-noto-sans-jp)" }}>
                              {r}
                ...
                <div>
                  <p style={{ fontFamily: "var(--font-inter)", fontSize: 10, fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 12 }}>
                    Example Words
                  </p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {item.examples.map((ex: string, i: number) => (
                      <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.08 }}
                        ...
                        <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--text-secondary)", marginTop: 6, flexShrink: 0 }} />
                        <p style={{ fontFamily: "var(--font-noto-sans-jp)", fontSize: 14, color: "var(--text-primary)", lineHeight: 1.6 }}>{ex}</p>
                      </motion.div>
                    ))}
                  </div>
                </div>
            ...
            <motion.button
              whileHover={{ scale: isLearned ? 1 : 1.02, background: isLearned ? "#10b981" : "var(--accent-red)" }}
              whileTap={{ scale: isLearned ? 1 : 0.98 }}
              onClick={handleComplete}
              style={{
                background: isLearned ? "#10b981" : "var(--text-primary)", color: "var(--bg-primary)",
  ...
  return (
    ...
    >
      <div style={{
        ...
        borderRadius: 18,
        border: isMastered ? "2px solid rgba(16, 185, 129, 0.5)" : "1px solid var(--border-color)",
        ...
        onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.boxShadow = "0 12px 40px rgba(0,0,0,0.06)"}
        onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.boxShadow = "0 2px 12px rgba(0,0,0,0.01)"}
      >
        {isLearned && (
          <div style={{
        ...
        <div style={{
          ...
          <div style={{ position: "absolute", top: "50%", left: 0, right: 0, height: 1, background: "var(--border-color)" }} />
          <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: 1, background: "var(--border-color)" }} />
          <span style={{ fontFamily: "var(--font-noto-sans-jp)", fontWeight: 100, fontSize: 46, color: "var(--text-primary)", lineHeight: 1, position: "relative" }}>
            {item.kanji}
          </span>
        </div>

        <p style={{ fontFamily: "var(--font-inter)", fontWeight: 500, fontSize: 13, color: "var(--text-primary)", textAlign: "center", lineHeight: 1.3 }}>
          {item.meaning}
        </p>

        <p style={{ fontFamily: "var(--font-inter)", fontSize: 10, color: "var(--text-secondary)", letterSpacing: "0.1em", textTransform: "uppercase" }}>
          {item.strokes} strokes
        </p>

        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, justifyContent: "center" }}>
          {item.onyomi.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(165, 180, 252, 0.1)", borderRadius: 99, padding: "3px 10px" }}>
              <span style={{ fontSize: 9, fontWeight: 600, color: "#a5b4fc", letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: "var(--font-inter)" }}>On</span>
              <span style={{ fontSize: 11, color: "#4338ca", fontFamily: "var(--font-noto-sans-jp)" }}>{item.onyomi.slice(0, 2).join("・")}</span>
            </div>
          )}
          {item.kunyomi.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(74, 222, 128, 0.1)", borderRadius: 99, padding: "3px 10px" }}>
              <span style={{ fontSize: 9, fontWeight: 600, color: "#86efac", letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: "var(--font-inter)" }}>Kun</span>
              <span style={{ fontSize: 11, color: "#166534", fontFamily: "var(--font-noto-sans-jp)" }}>{item.kunyomi.slice(0, 1).join("・")}</span>
            </div>
          )}
        </div>
      </div>
    ...
    >
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, overflow: "hidden" }}>
          ...
          {!isMobile && <span style={{ color: "var(--text-secondary)", fontSize: 13 }}>/</span>}
          {!isMobile && <span style={{ fontSize: 11, color: "var(--accent-red)", letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 600, whiteSpace: "nowrap" }}>N5 Kanji</span>}
        </div>
        <motion.button
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: isMobile ? "32px 24px 80px" : "56px 48px 80px" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div style={{
            ...
            <span style={{ fontSize: 10, fontWeight: 700, color: "var(--accent-red)", letterSpacing: "0.15em", textTransform: "uppercase" }}>JLPT N5</span>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 24, marginBottom: 32 }}>
            <div>
              <h1 style={{ fontFamily: "var(--font-cormorant)", fontStyle: "italic", fontWeight: 300, fontSize: "clamp(40px, 10vw, 64px)", color: "var(--text-primary)", lineHeight: 1.1 }}>
                N5 Kanji
              </h1>
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginTop: 8, fontWeight: 300 }}>
                Learn stroke order, meaning, and readings — <strong style={{ color: "var(--text-primary)", fontWeight: 500 }}>{kanjiData.length} kanji</strong> total
              </p>
            </div>
            ...
            <div style={{ display: "flex", gap: 0 }}>
              {[
                ...
                <div key={label} style={{
                  ...
                  <p style={{ fontFamily: "var(--font-cormorant)", fontSize: 32, fontWeight: 600, color: "var(--text-primary)", lineHeight: 1 }}>{value}</p>
          ...
          <div style={{ position: "relative", width: "100%", maxWidth: isMobile ? "none" : 420, marginBottom: 48 }}>
            <input
              ...
              onBlur={e => { e.target.style.borderColor = "var(--border-color)"; e.target.style.boxShadow = "0 1px 4px rgba(0,0,0,0.01)" }}
            />
            <svg style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", width: 16, height: 16, color: "var(--text-secondary)" }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {totalPages > 1 && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: isMobile ? "space-between" : "center", gap: 8, marginTop: 56, width: "100%" }}>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              ...
              disabled={page === 1}
              style={{
                padding: "8px 18px", borderRadius: 99, border: "1px solid var(--border-color)",
              ...
              Array.from({ length: totalPages }, (_, i) => i + 1).map(pg => (
                <motion.button key={pg} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                  ...
                  style={{
                    ...
                    border: "1px solid",
                    borderColor: pg === page ? "var(--text-primary)" : "var(--border-color)",
                    background: pg === page ? "var(--text-primary)" : "var(--bg-card)",
                    color: pg === page ? "var(--bg-primary)" : "var(--text-secondary)",
