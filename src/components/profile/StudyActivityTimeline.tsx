      ...
      {activityLogs.length === 0 ? (
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24, position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', left: 19, top: 20, bottom: 20, width: 2, background: "var(--bg-primary)", zIndex: 0 }} />

            {activityLogs.map((event) => (
              <div key={event.id} style={{ display: 'flex', gap: 16, position: 'relative', zIndex: 1 }}>
                <div style={{
                  width: 40, height: 40, borderRadius: '50%', background: "var(--bg-primary)", border: "1px solid var(--border-color)",
                ...
                <div style={{ flex: 1, paddingTop: 2 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                    <div>
                      <h3 style={{ fontSize: 14, fontWeight: 500, color: "var(--text-primary)", margin: '0 0 2px' }}>{event.title}</h3>
                      <p style={{ fontSize: 13, color: "var(--text-secondary)", margin: 0 }}>{event.description}</p>
                    </div>
                    <span style={{ fontSize: 12, color: "var(--text-secondary)", whiteSpace: 'nowrap' }}>{formatDate(event.created_at)}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          {activityLogs.length >= 20 && (
            <button style={{
              width: '100%', marginTop: 24, padding: '10px', background: 'transparent', border: "1px solid var(--border-color)",
